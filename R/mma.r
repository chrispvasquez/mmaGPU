  ...
  dist.m.given.x<-function(x,dirx,binm=NULL,contm=NULL,catm=NULL,nonlinear,df1,w,cova) #give the model and residual of m given x
{
  ...
  for (m in 1:length(full.model))
    ...
    if(class(full.model[[m]])[1] == "lgb.Booster"){
      new0 <- data.matrix(new0)
      new1 <- data.matrix(new1)
    }
    else{
      new0 <- new0_c
      new1 <- new1_c
    ...
    for(j in 1:ncol(y)){
      ...
      full.model[[j]]<-eval(parse(text=cf1))}
        else if(nonlinear)
        {
          if(distn == "bernoulli")
            distn <- "binary"
          else if (distn == "gaussian")
            distn <- "mean_squared_error"
          
          if (distn == "binary" || distn == "mean_squared_error"){
            ...
            set.seed(1)
            xy.sample <- sample(c(TRUE, FALSE), nrow(xy.df), replace=TRUE, prob=c(0.7,0.3))
            train_x2  <- xy.df[xy.sample, ]
            train_y2 <- train_x2$y2
            train_w <- train_x2$w
            train_x2<- train_x2[,!(names(train_x2) %in% c("y2", "w"))]
            
            test_x2  <- xy.df[!xy.sample, ]
            test_y2 <- test_x2$y2
            test_w <- test_x2$w
            test_x2<- test_x2[,!(names(test_x2) %in% c("y2", "w"))]
            
            train_lgb <- lgb.Dataset(data = unname(data.matrix(train_x2)), label = train_y2,
                                     colnames = colnames(x2), weight=train_w, free_raw_data=FALSE)
            
            test_lgb <- lgb.Dataset.create.valid(dataset = train_lgb, data = unname(data.matrix(test_x2)), 
                                                 label = test_y2, weight=test_w)
            
            valids_lgb <- list(test = test_lgb)
            
            full.model[[j]]<-suppressWarnings(lgb.train(data = train_lgb, params = learnParams, nrounds=num.trees, 
                                                        verbose = 0, valids = valids_lgb))
            
            #str(full.model[[j]])
            
            best.iter1[j]<-suppressWarnings(full.model[[j]]$best_iter)
            
            
            while(num.trees-best.iter1[j]<30){
              full.model[[j]]<-suppressWarnings(lgb.train(data = train_lgb, params = learnParams, nrounds=100, # do another 50 iterations
                                                          verbose = 0, init_model = full.model[[j]], reset_data = FALSE))
              best.iter1[j]<-suppressWarnings(full.model[[j]]$best_iter)
              num.trees <- num.trees + 100
            }
          }
 ...
 if(!is.null(data$bin.results)) {
   ...
   for (j in 1:ncol(y)){
     ...
     if(!is.null(custom.function)){
       ...
       {
         if(distn[j] == "bernoulli")
           distn[j] <- "binary"
         else if (distn[j] == "gaussian")
           distn[j] <- "mean_squared_error"
         
         if (distn[j] == "binary" || distn[j] == "mean_squared_error"){
           ...
           set.seed(1)
           xy.sample <- sample(c(TRUE, FALSE), nrow(xy.df), replace=TRUE, prob=c(0.7,0.3))
           train_x1  <- xy.df[xy.sample, ]
           train_y1 <- train_x1$y1
           train_w <- train_x1$w
           train_x1<- train_x1[,!(names(train_x1) %in% c("y1", "w"))]
           
           test_x1  <- xy.df[!xy.sample, ]
           test_y1 <- test_x1$y1
           test_w <- test_x1$w
           test_x1<- test_x1[,!(names(test_x1) %in% c("y1", "w"))]
           
           train_lgb <- lgb.Dataset(data = unname(data.matrix(train_x1)), label = train_y1,
                                    colnames = colnames(x1), weight=train_w, free_raw_data=FALSE)
           
           test_lgb <- lgb.Dataset.create.valid(dataset = train_lgb, data = unname(data.matrix(test_x1)), 
                                                label = test_y1, weight=test_w)
